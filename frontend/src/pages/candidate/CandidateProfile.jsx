        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-sm dark:shadow-none overflow-hidden">
            <div className="p-6 border-b border-slate-100 dark:border-zinc-800 flex items-center gap-3 bg-slate-50/50 dark:bg-zinc-800/50">
              <User className="w-5 h-5 text-indigo-600 dark:text-zinc-300" />
            ...
            <form onSubmit={handleProfileSubmit} className="p-6 space-y-6">
                ...
                {profileForm.skills.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {profileForm.skills.map((skill, index) => (
                      <span key={index} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 dark:bg-zinc-800 text-indigo-700 dark:text-zinc-300 font-bold text-xs rounded-lg border border-indigo-100 dark:border-zinc-700">
                        {skill}
                        <button type="button" onClick={() => removeSkill(index)} className="hover:text-indigo-900 dark:hover:text-zinc-100"><X className="w-3 h-3" /></button>
                      </span>
                    ))}
                  </div>
                )}
              ...
              <div className="pt-4 flex justify-end">
                <button type="submit" disabled={isSubmitting} className="h-10 px-6 font-bold text-white dark:text-zinc-900 bg-indigo-600 dark:bg-zinc-100 hover:bg-indigo-700 dark:hover:bg-white rounded-xl shadow-md transition-all active:scale-95 flex items-center gap-2 border border-transparent">
                  {isSubmitting ? <div className="w-4 h-4 border-2 border-indigo-400 dark:border-zinc-400 border-t-white dark:border-t-zinc-900 rounded-full animate-spin"></div> : <Save className="w-4 h-4" />}
                  Save Profile
                </button>
              </div>
            </form>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-sm dark:shadow-none overflow-hidden">
            ...
            <form onSubmit={handlePasswordSubmit} className="p-6 space-y-4">
              ...
              <div className="pt-2">
                <button type="submit" disabled={isPasswordSubmitting} className="w-full h-10 font-bold text-rose-700 dark:text-zinc-900 bg-rose-50 dark:bg-rose-400 border border-rose-100 dark:border-rose-500 hover:bg-rose-100 dark:hover:bg-rose-500 rounded-xl transition-all active:scale-95 flex justify-center items-center gap-2">
                  {isPasswordSubmitting ? <div className="w-4 h-4 border-2 border-rose-400 dark:border-zinc-400 border-t-rose-700 dark:border-t-zinc-900 rounded-full animate-spin"></div> : 'Reset Password'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-sm dark:shadow-none overflow-hidden flex flex-col">
            <div className="p-6 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between bg-slate-50/50 dark:bg-zinc-800/50">
              <div className="flex items-center gap-3">
                <FileText className="w-5 h-5 text-indigo-600 dark:text-zinc-300" />
              ...
              <label className={`cursor-pointer ${resumes.length >= 3 ? 'opacity-50 pointer-events-none' : ''}`}>
                ...
                <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-lg transition-colors border ${isUploadingResume ? 'bg-slate-100 dark:bg-zinc-800 text-slate-400 border-slate-200 dark:border-zinc-700' : 'bg-slate-900 dark:bg-zinc-100 text-white dark:text-zinc-900 border-slate-900 dark:border-zinc-100 hover:bg-slate-800 dark:hover:bg-white'}`}>
                  {isUploadingResume ? (
