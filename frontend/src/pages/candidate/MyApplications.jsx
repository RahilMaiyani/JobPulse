...
export default function MyApplications() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        <h1 className="text-3xl font-black text-slate-900 dark:text-zinc-100 tracking-tight">My Applications</h1>
        <p className="text-slate-500 dark:text-zinc-400 font-medium mt-1">Track the status of your recent job applications.</p>
      </div>

      {loading ? (
        <MyApplicationsSkeleton count={5} />
      ) : applications.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl shadow-sm dark:shadow-none">
          <Briefcase className="w-12 h-12 text-slate-300 dark:text-zinc-700 mx-auto mb-4" />
          <h3 className="text-lg font-bold text-slate-900 dark:text-zinc-100">No applications yet</h3>
          <p className="text-slate-500 dark:text-zinc-500 text-sm mt-1">Head over to Open Roles to find your next opportunity.</p>
        </div>
      ) : (
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl shadow-sm dark:shadow-none overflow-hidden">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
                {applications.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((app) => (
                  <tr
                    key={app.id}
                    className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/30 transition-colors cursor-pointer"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-xs font-medium text-slate-500 dark:text-zinc-500">{app.company_name || 'Company'}</span>
                            <span className="w-1 h-1 rounded-full bg-slate-300 dark:bg-zinc-700"></span>
                            <span className="text-xs font-bold text-slate-400 dark:text-zinc-500">{app.job_type}</span>
                          </div>
                        </div>
                      </div>
                    </td>
