...
export default function AdminDashboard() {
  ...
  return (
    <DashboardLayout>
      {/* HEADER SECTION */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8 pb-6 border-b border-zinc-200/60 dark:border-zinc-800/60">
        <div>
          <div className="inline-flex items-center rounded-full border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 px-3 py-1 text-[10px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-widest mb-3 shadow-sm">
            <span className="flex h-1.5 w-1.5 rounded-full bg-indigo-500 mr-2 animate-pulse"></span>
            Control Center
          </div>
          <h1 className="text-3xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight">Overview</h1>
          <p className="text-zinc-500 dark:text-zinc-400 text-sm font-semibold mt-1">Here is what's happening today, {user?.full_name?.split(' ')[0]}.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Link 
            to="/admin/users"
            className="inline-flex items-center justify-center h-10 px-4 text-xs font-bold text-zinc-700 dark:text-zinc-300 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800 rounded-xl shadow-sm transition-all active:scale-95 gap-2"
          >
          ...
          <Link 
            to="/admin/jobs"
            className="inline-flex items-center justify-center h-10 px-4 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-md shadow-indigo-500/20 transition-all active:scale-95 gap-2"
          >
              ...
              <div key={i} className="relative overflow-hidden bg-white dark:bg-zinc-950 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm dark:shadow-none group hover:border-zinc-300 dark:hover:border-zinc-750 hover:shadow-md transition-all duration-350">
                ...
                <div className="relative z-10 flex items-center justify-between">
                  <div className="space-y-2">
                    <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest leading-none">{stat.label}</p>
                    <div className="flex items-baseline gap-2 mt-1">
                      <p className="text-3xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight leading-none">{stat.value}</p>
                ...
                <React.Fragment key={i}>
                  <div className="flex items-center gap-4 w-full lg:w-auto flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest leading-none">{stage.label}</p>
                      <div className="flex items-baseline gap-2 mt-1.5">
                        <span className="text-2xl font-black text-zinc-900 dark:text-zinc-100 leading-none">{stage.count}</span>
                        <span className="text-[9px] font-bold text-zinc-400 dark:text-zinc-500 truncate uppercase tracking-wider">candidates</span>
                      </div>
                    </div>
                  </div>
                  {i < funnelStages.length - 1 && (
                    <div className="hidden lg:block text-zinc-200 dark:text-zinc-800 font-light text-xl px-1 select-none">➔</div>
                  )}
                </React.Fragment>
              ))}
            </div>
