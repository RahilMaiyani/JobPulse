...
export default function ScheduleInterviewModal({ job, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl shadow-2xl w-full max-w-4xl relative z-10 overflow-hidden flex flex-col max-h-[90vh]">
        
        <div className="p-6 md:p-8 border-b border-slate-100 dark:border-zinc-800 flex justify-between items-start shrink-0">
          <div>
            <h2 className="text-2xl font-black text-slate-900 dark:text-zinc-100 tracking-tight">Schedule Interviews</h2>
            <p className="text-sm font-bold text-slate-500 dark:text-zinc-400 mt-1">{job.title}</p>
          </div>
          <button onClick={onClose} className="p-2 text-slate-400 dark:text-zinc-500 hover:text-slate-900 dark:hover:text-zinc-100 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 md:p-8 flex-1 overflow-y-auto custom-scrollbar">
          {loadingApps || loadingInterviews ? (
            ...
            <div className="text-center py-12">
              <p className="text-slate-500 font-medium">No candidates are currently eligible or shortlisted for an interview.</p>
            </div>
          ) : (
            <div className="space-y-6">
              {candidates.map((app) => {
                ...
                return (
                  <div key={app.id} className="p-5 rounded-2xl border border-slate-200 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-800/30">
                    <div className="flex flex-col md:flex-row justify-between gap-4">
                      <div className="flex-1">
                        <h4 className="font-bold text-lg text-slate-900 dark:text-zinc-100">{app.candidate_name}</h4>
                        <p className="text-xs font-medium text-slate-500 dark:text-zinc-400">{app.candidate_email}</p>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                          <div>
                            <label className="block text-[10px] font-black text-slate-500 dark:text-zinc-400 uppercase tracking-widest mb-1">Date</label>
                            <div className="relative">
                              ...
                              <input 
                                ...
                                className="w-full pl-9 pr-3 py-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl text-sm font-medium focus:ring-2 focus:ring-indigo-500 outline-none"
                              />
                            </div>
                          </div>
                          <div>
                            <label className="block text-[10px] font-black text-slate-500 dark:text-zinc-400 uppercase tracking-widest mb-1">Time</label>
                            <div className="relative">
                          ...
                          <div className="md:col-span-2">
                            <label className="block text-[10px] font-black text-slate-500 dark:text-zinc-400 uppercase tracking-widest mb-1">Interviewer Notes / Link (Optional)</label>
                            <div className="relative">
                              <Edit3 className="absolute left-3 top-3 w-4 h-4 text-slate-400" />
                              <textarea 
                                value={schedule.notes}
                                onChange={(e) => handleUpdateSchedule(app.id, 'notes', e.target.value)}
                                placeholder="E.g., Google Meet link, topics to cover..."
                                className="w-full pl-9 pr-3 py-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl text-sm font-medium min-h-[60px] focus:ring-2 focus:ring-indigo-500 outline-none"
                              />
                            </div>
                          </div>
                        </div>
