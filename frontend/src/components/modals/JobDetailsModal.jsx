...
export default function JobDetailsModal({ job, onClose, onApply, appliedApplication }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-950/40 dark:bg-black/60 backdrop-blur-sm" onClick={onClose}></div>
      <div className="bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl w-full max-w-3xl relative z-10 overflow-hidden animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col border border-transparent dark:border-zinc-800">
        <div className="p-6 md:p-8 border-b border-slate-100 dark:border-zinc-800 flex justify-between items-start bg-slate-50/50 dark:bg-zinc-800/50">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-900 dark:text-zinc-100 tracking-tight leading-tight">{job.title}</h2>
              <div className="flex flex-wrap items-center gap-3 mt-3 text-sm font-bold text-slate-500 dark:text-zinc-400">
        ...
        <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar flex-1 bg-white dark:bg-zinc-900">
          <div className="prose prose-slate dark:prose-invert max-w-none">
            <h3 className="text-lg font-black text-slate-900 dark:text-zinc-100 mb-4">Role Overview</h3>
            <p className="text-slate-600 dark:text-zinc-300 leading-relaxed font-medium whitespace-pre-wrap">{job.description}</p>

            {job.requirements && (
              <>
                <div className="w-full h-px bg-slate-100 dark:bg-zinc-800 my-8"></div>
                <h3 className="text-lg font-black text-slate-900 dark:text-zinc-100 mb-4">Requirements & Skills</h3>
                <ul className="space-y-3">
                  {typeof job.requirements === 'object' && Array.isArray(job.requirements)
                    ? job.requirements.map((req, i) => (
                      <li key={i} className="flex items-start gap-3 text-slate-600 dark:text-zinc-300 font-medium">
                        <CheckCircle2 className="w-5 h-5 text-emerald-500 dark:text-emerald-400 shrink-0 mt-0.5" />
                        <span>{req}</span>
                      </li>
                    ))
                    : <li className="flex items-start gap-3 text-slate-600 dark:text-zinc-300 font-medium">
                      <CheckCircle2 className="w-5 h-5 text-emerald-500 dark:text-emerald-400 shrink-0 mt-0.5" />
        ...
        <div className="p-6 border-t border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex justify-between items-center">
          <div className="text-sm font-bold text-slate-500 dark:text-zinc-400">
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className="px-6 h-12 font-bold text-slate-600 dark:text-zinc-300 hover:text-slate-900 dark:hover:text-zinc-100 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-xl transition-colors"
            >
              Close
            </button>
            {appliedApplication ? (
              ...
              <button
                disabled
                className="px-8 h-12 font-bold text-slate-400 dark:text-zinc-500 bg-slate-100 dark:bg-zinc-800 rounded-xl flex items-center gap-2 cursor-not-allowed"
              >
                Expired
              </button>
            ) : (
