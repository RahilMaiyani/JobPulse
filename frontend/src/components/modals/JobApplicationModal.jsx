        ...
        <div className="p-4 sm:p-6 md:p-8 overflow-y-auto custom-scrollbar bg-white dark:bg-zinc-900">
          ...
          {analysisState === 'idle' && (
            <div className="space-y-6">
              <div>
                <h3 className="text-lg font-black text-zinc-900 dark:text-zinc-100 mb-2">Select a Resume</h3>
                <p className="text-sm text-zinc-500 dark:text-zinc-400 font-medium">Choose a resume from your vault or upload a new one.</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {resumes.map(r => (
                  <div
                    ...
                    className={`p-4 border-2 rounded-xl cursor-pointer transition-all ${selectedResumeId === r.id ? 'border-indigo-600 dark:border-indigo-500 bg-indigo-50 dark:bg-indigo-500/10 shadow-md shadow-indigo-100 dark:shadow-none' : 'border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800/50'}`}
                  >
          ...
          {analysisState === 'analyzing' && (
            <div className="py-20 flex flex-col items-center justify-center text-center">
              <div className="relative mb-6">
                ...
                <div className="w-16 h-16 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 shadow-xl rounded-2xl flex items-center justify-center relative z-10 animate-bounce">
                  <Sparkles className="w-8 h-8 text-indigo-600 dark:text-indigo-400" />
                </div>
              </div>
              <h3 className="text-xl font-black text-zinc-900 dark:text-zinc-100">Gemini is analyzing your resume...</h3>
              <p className="text-zinc-500 dark:text-zinc-400 font-medium mt-2 max-w-sm">We are cross-referencing your experience and skills with the job requirements. This will just take a moment.</p>
            </div>
          )}
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1 space-y-4">
                  <div>
                    <h4 className="text-sm font-black text-zinc-900 dark:text-zinc-100 mb-2 flex items-center gap-2">
                      <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Strengths
                    </h4>
                    <ul className="space-y-1">
                      {analysisResult.ai_match_details.strengths.map((s, i) => (
                        <li key={i} className="text-sm text-zinc-600 dark:text-zinc-400 font-medium leading-relaxed">• {s}</li>
                      ))}
                    </ul>
                  </div>
                    ...
                    <div>
                      ...
                      <ul className="space-y-1">
                        {analysisResult.ai_match_details.weaknesses.map((w, i) => (
                          <li key={i} className="text-sm text-zinc-600 dark:text-zinc-400 font-medium leading-relaxed">• {w}</li>
                        ))}
                      </ul>
                    </div>
                  )}
              ...
              <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-xl border border-zinc-100 dark:border-zinc-800">
                <p className="text-sm text-zinc-700 dark:text-zinc-300 italic font-medium">"{analysisResult.ai_match_details.reasoning}"</p>
              </div>
