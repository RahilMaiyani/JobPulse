...
export default function ArchiveModal({ isOpen, onClose, onConfirm, title, isArchiving }) {
  return (
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="space-y-4 mb-8">
          <p className="text-sm text-slate-500 font-medium leading-relaxed">
            Are you sure you want to archive <span className="text-slate-900 font-bold">"{title}"</span>?
          </p>
          
          <div className="flex items-center gap-2 p-3 bg-slate-50 border border-slate-100 rounded-lg">
            <AlertCircle className="w-4 h-4 text-slate-400" />
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
              This will remove it from all employee feeds
            </p>
          </div>
        </div>
